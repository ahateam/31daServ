...
public class RDSUtils {
	...
	public static void createTableByEntity(DataSource ds, Class entityClass) throws Exception {
		createTable(ds, entityClass);
		createIndex(ds, entityClass);
	}

	public static void dropTableByEntity(DataSource ds, Class entityClass) throws Exception {
		...
		if (null == annEntity) {
			...
			for (Field classField : classFields) {
				if (!Modifier.isStatic(classField.getModifiers())) {
					...
					zyxhj.org.cn.utils.data.rds.RDSAnnField annField = classField
							.getAnnotation(zyxhj.org.cn.utils.data.rds.RDSAnnField.class);
